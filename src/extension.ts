...
function getWebviewContent(webview: vscode.Webview, context: vscode.ExtensionContext, jsonContent: string): string {
    ...
    // --- UPDATED PATHS TO POINT TO 'out' ---
    const htmlPath = vscode.Uri.joinPath(context.extensionUri, 'out', 'webview', 'main.html');
    const scriptPath = vscode.Uri.joinPath(context.extensionUri, 'out', 'webview', 'main.js');
    const stylePath = vscode.Uri.joinPath(context.extensionUri, 'out', 'webview', 'main.css');
    // ---
